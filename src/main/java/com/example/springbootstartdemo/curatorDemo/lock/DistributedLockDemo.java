/*
package com.example.springbootstartdemo.curatorDemo.lock;
...
public class DistributedLockDemo {
    ...
    public void init() throws Exception {
        ...
        client.start();
        client2.start();
        System.out.println("-------------建立连接--------------- client:"+client+",client2:"+client2);
    }
...
　　InterProcessSemaphoreMutex是一种不可重入的互斥锁，
   也就意味着即使是同一个线程也无法在持有锁的情况下再次获得锁，所以需要注意，不可重入的锁很容易在一些情况导致死锁。*//*
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    System.out.println(lock.acquire(2, TimeUnit.SECONDS));//false
                    // interProcessLock1 是可重入锁, 所以可以继续获取锁
                    System.out.println(interProcessLock1.acquire(2, TimeUnit.SECONDS));//true
                    // interProcessLock2 是不可重入锁, 所以获取锁失败
                    System.out.println(interProcessLock2.acquire(2, TimeUnit.SECONDS));//false
